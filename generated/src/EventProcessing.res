...
let rec registerDynamicContracts = (
  ...
  | list{eventBatchQueueItem, ...tail} =>
    let dynamicContractRegistrationsResult = if (
      ...
      | Greeter_NewGreeting(event) => runEventContractRegister(Greeter_NewGreeting, event)
      | Greeter_ClearGreeting(event) => runEventContractRegister(Greeter_ClearGreeting, event)
      }
    }
...
let runLoaders = (
  ...
  runAsyncEnv(async () => {
    ...
    let loaderReturnsUnawaited = eventBatch->Array.keepMap(({chain, event}) => {
      ...
      switch event {
      | Greeter_NewGreeting(event) => event->load(Greeter_NewGreeting)
      | Greeter_ClearGreeting(event) => event->load(Greeter_ClearGreeting)
      }
    })
...
let runHandlers = (
  ...
  runAsyncEnv(async () => {
    for i in 0 to eventBatch->Array.length - 1 {
      ...
      latestProcessedBlocks :=
        switch event {
        | Greeter_NewGreeting(event) =>
          await event
          ->runHandler(module(Types.Greeter.NewGreeting))
          ->propogateAsync
        | Greeter_ClearGreeting(event) =>
          await event
          ->runHandler(module(Types.Greeter.ClearGreeting))
          ->propogateAsync
        }
    }
